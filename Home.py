...
def load_ner_model():
    ...
    try:    
        ...
        st.write(f"🔍 Loading NER model from {model_path}...")
        
        # Add retry logic with visible feedback
        max_retries = 2
        for attempt in range(max_retries + 1):
            try:
                if attempt > 0:
                    print(f"🔄 NER DEBUG: Retry attempt {attempt}/{max_retries}")
                    st.write(f"🔄 Connection retry {attempt}/{max_retries} for NER model...")
                ...
                is_connection_error = any(keyword in error_msg for keyword in [
                    'timeout', 'connection', 'network', 'http', 'ssl', 'resolve', 
                    '429', 'rate limit', 'max retries', 'timed out'
                ])
                
                if is_connection_error and attempt < max_retries:
                    print(f"🌐 NER DEBUG: Connection issue, retrying... ({download_error})")
                    st.write(f"🌐 Connection issue detected: {str(download_error)[:100]}...")
                    st.write(f"⏳ Waiting 5 seconds before retry {attempt + 1}/{max_retries}...")
                    import time
                    time.sleep(5)
                    continue
                elif is_connection_error:
                    print(f"🌐 NER DEBUG: HuggingFace connection issue after {max_retries + 1} attempts: {download_error}")
                    st.write(f"❌ Failed to connect to HuggingFace after {max_retries + 1} attempts")
                    st.write(f"🔍 Final error: {str(download_error)}")
                    raise ConnectionError(f"Could not connect to Hugging Face to download NER model after {max_retries + 1} attempts. Error: {download_error}")
                else:
                    print(f"❌ NER DEBUG: Model loading error: {download_error}")
                    st.write(f"❌ NER model loading error: {str(download_error)}")
                    raise
...
def load_stage2_model():
    ...
    try:
        ...
        st.write(f"🔍 Loading Stage 2 tokenizer from {model_path}...")
        
        # Load tokenizer with retry logic and visible feedback
        max_retries = 2
        for attempt in range(max_retries + 1):
            try:
                if attempt > 0:
                    print(f"🔄 STAGE2 DEBUG: Tokenizer retry attempt {attempt}/{max_retries}")
                    st.write(f"🔄 Tokenizer connection retry {attempt}/{max_retries}...")
                
                tokenizer = AutoTokenizer.from_pretrained(model_path)
                print("🔍 STAGE2 DEBUG: Tokenizer loaded successfully")
                st.write("✅ Stage 2 tokenizer loaded successfully!")
                break
                ...
                if is_connection_error and attempt < max_retries:
                    ...
                    st.write(f"🌐 Tokenizer connection issue: {str(download_error)[:100]}...")
                    st.write(f"⏳ Waiting 3 seconds before tokenizer retry...")
                    import time
                    time.sleep(3)
                    continue
                elif is_connection_error:
                    print(f"🌐 STAGE2 DEBUG: HuggingFace tokenizer connection issue after {max_retries + 1} attempts: {download_error}")
                    st.write(f"❌ Failed to connect for Stage 2 tokenizer after {max_retries + 1} attempts")
                    st.write(f"🔍 Tokenizer error: {str(download_error)}")
                    raise ConnectionError(f"Could not connect to Hugging Face to download Stage 2 tokenizer after {max_retries + 1} attempts. Error: {download_error}")
                else:
                    print(f"❌ STAGE2 DEBUG: Tokenizer loading error: {download_error}")
                    st.write(f"❌ Stage 2 tokenizer error: {str(download_error)}")
                    raise
        
        print(f"🔍 STAGE2 DEBUG: About to load model from {model_path}")
        st.write(f"🔍 Loading Stage 2 classification model from {model_path}...")
        
        # Load model with retry logic and visible feedback
        for attempt in range(max_retries + 1):
            try:
                if attempt > 0:
                    print(f"🔄 STAGE2 DEBUG: Model retry attempt {attempt}/{max_retries}")
                    st.write(f"🔄 Stage 2 model connection retry {attempt}/{max_retries}...")
                
                model = AutoModelForSequenceClassification.from_pretrained(model_path)
                print("🔍 STAGE2 DEBUG: Model loaded successfully")
                st.write("✅ Stage 2 classification model loaded successfully!")
                break
                ...
                if is_connection_error and attempt < max_retries:
                    ...
                    st.write(f"🌐 Stage 2 model connection issue: {str(download_error)[:100]}...")
                    st.write(f"⏳ Waiting 3 seconds before model retry...")
                    import time
                    time.sleep(3)
                    continue
                elif is_connection_error:
                    print(f"🌐 STAGE2 DEBUG: HuggingFace model connection issue after {max_retries + 1} attempts: {download_error}")
                    st.write(f"❌ Failed to connect for Stage 2 model after {max_retries + 1} attempts")
                    st.write(f"🔍 Stage 2 model error: {str(download_error)}")
                    raise ConnectionError(f"Could not connect to Hugging Face to download Stage 2 classification model after {max_retries + 1} attempts. Error: {download_error}")
                else:
...
if NER_MODEL is not None and STAGE2_MODEL is not None:
    ...
    if 'prediction_error' not in locals():  # If not set by ConnectionError
